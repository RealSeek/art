...
const AgentState = Annotation.Root({
  ...
  verdict: Annotation<'complete' | 'replan' | 'partial'>(),
...
export class AgentTasksProcessor extends WorkerHost {
  ...
  async process(job: Job<{ taskId: string; runId: string; runKey: string }>) {
    ...
    try {
      ...
      const graph = new StateGraph(AgentState)
        ...
        .addConditionalEdges('verify', (state) => state.verdict, { complete: 'deliver', partial: 'deliver', replan: 'replan' })
  ...
  private async plan(state: typeof AgentState.State) {
    await this.assertActive(state.taskId)
    await this.assertBudget(state.runId)
    ...
    const completion = await this.completeModel(state, task, `planner-${state.iteration}`, prompt, async () => this.touch(state.taskId, state.runId, '正在生成结构化执行计划'))
  ...
  private async executeTools(state: typeof AgentState.State) {
    ...
    for (const call of calls) {
      ...
      try { await this.reserveToolBudget(state.runId, call.key === 'web_search') }
      catch (reason) {
        const error = reason instanceof Error ? reason.message : 'Agent 工具预算已用尽'
        await this.prisma.agentToolCall.update({ where: { id: call.id }, data: { status: 'SKIPPED', error, completedAt: new Date() } })
        results.push({ tool: call.key, name: call.name, status: 'SKIPPED', error })
        await this.event(state.taskId, state.runId, 'budget', '已停止继续调用工具', error)
        break
      }
  ...
  private async verify(state: typeof AgentState.State) {
    ...
    const completion = await this.completeModel(state, task, `verifier-${state.iteration}`, prompt)
    ...
    const exhausted = !approved && state.iteration + 1 >= run.maxIterations
    const nextVerdict = approved ? 'complete' as const : exhausted ? 'partial' as const : 'replan' as const
    await this.prisma.agentRun.update({ where: { id: state.runId }, data: { verifierFeedback: feedback, currentNode: nextVerdict === 'replan' ? 'replan' : 'deliver' } })
    await this.event(state.taskId, state.runId, 'verification', approved ? '结果校验完成' : exhausted ? '结果部分完成' : '需要继续完善', feedback || '已通过交付质量检查')
    return { verdict: nextVerdict, verifierFeedback: feedback }
  }
  ...
  private async replan(state: typeof AgentState.State) {
    ...
    const task = await this.task(state.taskId)
    const available = await this.tools.available(task)
    const completion = await this.completeModel(state, task, `planner-${nextIteration}`, this.plannerPrompt(task.goal, this.taskInstructions(task), available, state.verifierFeedback, nextIteration))
  ...
  private async deliver(state: typeof AgentState.State) {
    ...
    const task = await this.task(state.taskId)
    const now = new Date()
    let partial = state.verdict === 'partial'
    let deliveryFeedback = state.verifierFeedback
    ...
    const message = existing
      ? await this.prisma.message.update({ where: { id: existing.id }, data: { content: state.answer, model: task.model }, select: { id: true } })
      : await this.prisma.message.create({ data: { conversationId: task.conversationId!, role: 'ASSISTANT', content: state.answer, model: task.model, metadata: { agentTaskId: task.id, agentRunId: state.runId } }, select: { id: true } })
    try { await this.officeExports.create(task.userId, { agentTaskId: task.id }) }
    catch (reason) {
      partial = true
      const message = reason instanceof Error ? reason.message : '办公文件生成失败'
      deliveryFeedback = [deliveryFeedback, `文件交付失败：${message}`].filter(Boolean).join('\n').slice(0, 8000)
      await this.event(state.taskId, state.runId, 'artifact_error', '交付文件生成失败', message)
    }
    await this.prisma.message.update({ where: { id: message.id }, data: { metadata: { agentTaskId: task.id, agentRunId: state.runId, partial, verifierFeedback: deliveryFeedback } } })
    const finalStatus = partial ? AgentTaskStatus.PARTIAL : AgentTaskStatus.SUCCEEDED
    await this.prisma.$transaction([
      this.prisma.agentTaskStep.updateMany({ where: { agentTaskId: state.taskId, position: 3 }, data: { status: partial ? AgentTaskStepStatus.FAILED : AgentTaskStepStatus.SUCCEEDED, detail: partial ? `任务部分完成：${deliveryFeedback.slice(0, 1000)}` : `通过 ${state.iteration + 1} 轮执行与校验，已生成最终结果和办公文件`, completedAt: now } }),
      this.prisma.agentRun.update({ where: { id: state.runId }, data: { status: finalStatus, currentNode: 'done', finalAnswer: state.answer, completedAt: now } }),
      this.prisma.agentTask.update({ where: { id: state.taskId }, data: { status: finalStatus, errorMessage: partial ? deliveryFeedback.slice(0, 2000) : null, completedAt: now } }),
      this.prisma.conversation.update({ where: { id: task.conversationId! }, data: { updatedAt: now } }),
    ])
    await this.event(state.taskId, state.runId, partial ? 'partial' : 'completed', partial ? '任务部分完成' : '任务已完成', partial ? deliveryFeedback : `经过 ${state.iteration + 1} 轮执行与校验，办公文件已写入文件库`)
  ...
  private async assertBudget(runId: string) {
    const run = await this.prisma.agentRun.findUniqueOrThrow({ where: { id: runId }, select: { deadlineAt: true, maxCreditCost: true, creditCost: true } })
    if (run.deadlineAt && run.deadlineAt.getTime() <= Date.now()) throw new Error('Agent 任务已超过最大执行时间')
    if (run.creditCost >= run.maxCreditCost) throw new Error(`Agent 任务已达到 ${run.maxCreditCost} 点执行预算`)
  }

  private async reserveToolBudget(runId: string, search: boolean) {
    const run = await this.prisma.agentRun.findUniqueOrThrow({ where: { id: runId }, select: { maxToolCalls: true, maxSearchQueries: true, toolCallCount: true, searchQueryCount: true, deadlineAt: true } })
    if (run.deadlineAt && run.deadlineAt.getTime() <= Date.now()) throw new Error('Agent 任务已超过最大执行时间')
    if (run.toolCallCount >= run.maxToolCalls) throw new Error(`工具调用已达到 ${run.maxToolCalls} 次上限`)
    if (search && run.searchQueryCount >= run.maxSearchQueries) throw new Error(`联网搜索已达到 ${run.maxSearchQueries} 次上限`)
    await this.prisma.agentRun.update({ where: { id: runId }, data: { toolCallCount: { increment: 1 }, ...(search ? { searchQueryCount: { increment: 1 } } : {}) } })
  }

  private async completeModel(
    state: Pick<typeof AgentState.State, 'taskId' | 'runId' | 'runKey'>,
    task: Awaited<ReturnType<AgentTasksProcessor['task']>>,
    purpose: string,
    prompt: string,
    onProgress?: (content: string) => Promise<void>,
  ) {
    await this.assertBudget(state.runId)
    const completion = await this.model.complete(task, state.runKey, purpose, prompt, onProgress)
    await this.prisma.agentRun.update({ where: { id: state.runId }, data: { creditCost: { increment: Math.max(0, completion.creditCost || 0) } } })
    return completion
  }
