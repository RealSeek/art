...
export class AdminRolesController {
  ...
  @Post() async create(@CurrentUser() admin: AuthenticatedUser, @Req() request: FastifyRequest, @Body() body: RoleDto) { const result = await this.roles.create(body); await this.audit(admin.id, request, 'admin.role.create', result.id, result); return result }
  @Patch('administrators/:userId') async assign(@CurrentUser() admin: AuthenticatedUser, @Req() request: FastifyRequest, @Param('userId') userId: string, @Body() body: AssignRoleDto) { const result = await this.roles.assign(userId, body.adminRoleId || null, admin.id); await this.audit(admin.id, request, 'admin.role.assign', userId, result); return result }
  @Patch(':id') async update(@CurrentUser() admin: AuthenticatedUser, @Req() request: FastifyRequest, @Param('id') id: string, @Body() body: UpdateRoleDto) { const result = await this.roles.update(id, body); await this.audit(admin.id, request, 'admin.role.update', id, result); return result }
  @Delete(':id') async remove(@CurrentUser() admin: AuthenticatedUser, @Req() request: FastifyRequest, @Param('id') id: string) { const result = await this.roles.remove(id); await this.audit(admin.id, request, 'admin.role.delete', id, result); return result }
