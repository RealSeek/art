<template>
  ...
  <section class="settings-token-quota"><header><div><h3>文字额度</h3><p>对话按实际输入和输出 Token 结算，图片、视频和商品视觉仍使用创作点。</p></div><span v-if="tokenQuotaLoading">加载中</span></header><div v-if="tokenQuotaRows.length" class="settings-token-quota-list"><article v-for="quota in tokenQuotaRows" :key="quota.quotaId || quota.scopeKey || 'quota'"><div><strong>{{ quota.isDaily ? '今日额度' : '本周期额度' }}</strong><small>{{ quota.periodEnd ? `${quota.periodEnd} 前有效` : '当前有效额度' }}</small></div><dl><div><dt>剩余</dt><dd>{{ formatUnits(quota.remainingUnits) }}</dd></div><div><dt>已用</dt><dd>{{ formatUnits(quota.usedUnits) }}</dd></div><div><dt>预留</dt><dd>{{ formatUnits(quota.reservedUnits) }}</dd></div><div><dt>总额</dt><dd>{{ formatUnits(quota.grantedUnits) }}</dd></div></dl></article></div><p v-else-if="!tokenQuotaLoading" class="settings-token-quota-empty">暂未分配文字额度，升级套餐后即可使用。</p></section>
