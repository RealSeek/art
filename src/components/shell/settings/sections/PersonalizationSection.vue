<template>
  ...
  <section class="settings-about"><h3>关于你</h3><p>这些资料会持续用于个性化回复。请勿填写密码、密钥、证件号或支付账户。</p><label>昵称<input v-model="settings.nickname" placeholder="OnlyArt 应该怎么称呼你？" /></label><label>职业<input v-model="settings.occupation" placeholder="例如：独立开发者" /></label><label>你的详情<textarea v-model="settings.bio" maxlength="1000" placeholder="需要持续考虑的兴趣、目标、工作方式或背景" /></label><button type="button" @click="saveSettings(true)">保存</button><small v-if="settingsMessage" class="settings-feedback">{{ settingsMessage }}</small></section>
