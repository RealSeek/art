...
export interface CredentialEditor { id?: string; apiKey: string; apiKeyHint: string; enabled: boolean; isDefault: boolean; priority: number; weight: number; expiresAt: string; autoImport: boolean }
...
export interface AvailableModel { key: string; displayName: string; capability: 'CHAT' | 'IMAGE' | 'VIDEO' }
