    ...
    <section class="login-panel">
      <header class="login-header">
        ...
        <div class="login-header__tools">
          ...
          <span class="login-account-pill">New API 账户</span>
        </div>
      </header>
      ...
      <section class="login-form" aria-live="polite">
        ...
        <div v-else class="auth-step auth-step--multi">
          <div class="login-heading">
            <h1>欢迎回来</h1>
            <p>使用现有 New API 账户继续，无需单独注册 OnlyArt 账户。</p>
          </div>
          <p v-if="error" class="login-error" role="alert">{{ error }}</p>
          <a class="button button--solid login-submit" :href="newApiLoginUrl">使用 New API 登录</a>
        </div>
      </section>
...
const error = computed(() => route.query.error === 'new-api' ? 'New API 登录失败，请重试。' : '')
