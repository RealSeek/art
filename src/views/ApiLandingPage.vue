  ...
  <main class="api-landing" :class="{ 'is-dark': dark }">
    ...
    <section id="top" class="api-hero">
      <div class="api-hero-copy">
        ...
        <div class="api-hero-actions"><a v-if="apiEntries.length" class="is-primary" href="#console">{{ copy.start }}<ArrowRight :size="17" /></a><RouterLink v-else class="is-primary" to="/chat">{{ copy.workspace }}<ArrowRight :size="17" /></RouterLink><RouterLink to="/chat?settings=api">{{ copy.pricing }}</RouterLink></div>
...
const copy = computed(() => english.value ? {
  home: 'Home', console: 'API access', about: 'About', language: 'Switch language', theme: 'Switch theme', notifications: 'Notifications', workspace: 'Workspace', login: 'Sign in', kicker: 'AI service access', title: 'Configured access for', titleAccent: 'your AI services', description: 'API access is supplied through administrator-configured services. Choose an available entry to continue.', start: 'Open API service', pricing: 'API key', apps: 'Available services', appsDetail: 'Entries are managed by the workspace administrator', unavailable: 'No API service configured', ready: 'Available', entryFallback: 'Open the configured service', emptyTitle: 'API access is not configured', emptyText: 'An administrator can add an API service from the management console.', configuredByAdmin: 'Managed by administrator', entries: 'entries', noEntry: 'No entry', pricingText: 'Create an API key in OnlyCode and configure it in your workspace.', entryText: 'Configured API services open from this page without claiming unsupported local routes.', aboutText: 'OnlyArt provides a workspace for AI creation and configured service access.',
} : {
  home: '主页', console: 'API 服务', about: '关于', language: '更改语言', theme: '切换主题', notifications: '通知', workspace: '进入工作台', login: '登录', kicker: 'AI 服务接入', title: '面向已配置的', titleAccent: 'AI 服务入口', description: 'API 服务由管理员在后台配置。请选择可用入口继续，不再展示未部署的本地接口。', start: '打开 API 服务', pricing: '配置密钥', apps: '可用服务', appsDetail: '入口由工作台管理员统一管理', unavailable: '暂未配置 API 服务', ready: '可用', entryFallback: '打开已配置的服务', emptyTitle: '暂未配置 API 服务', emptyText: '管理员可在管理端的“外部入口”中添加 API 服务地址。', configuredByAdmin: '由管理员配置', entries: '个入口', noEntry: '无可用入口', pricingText: '在 OnlyCode 创建 API 密钥后，将其配置到工作台即可使用。', entryText: '本页只打开管理员配置的 API 服务，不再宣称未部署的本地兼容接口。', aboutText: 'OnlyArt 提供 AI 创作工作台和管理员配置的服务接入。',
})
