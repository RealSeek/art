  ...
  <main class="plugin-market-page">
    <div class="plugin-market-inner">
      <header class="plugin-market-header">
        <div><h1>插件市场</h1><p>安装官方插件，或创建仅自己可用的私有插件</p></div>
        <button v-if="activeTab === 'mine'" class="plugin-primary-action" type="button" @click="openEditor()"><Plus :size="16" />新建私有插件</button>
      </header>

      <nav class="plugin-tabs" aria-label="插件视图">
      ...
      <section v-if="activeTab === 'market'" class="plugin-filter-bar">
        <label><Search :size="15" /><input v-model="query" placeholder="搜索插件" /></label>
      ...
      <section v-if="loading" class="plugin-empty"><LoaderCircle class="plugin-spin" :size="22" /><span>正在加载插件</span></section>
